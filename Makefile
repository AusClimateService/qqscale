...
${AF_FILE} :
	${PYTHON} ${CODE_DIR}/calc_adjustment.py ${MODEL_VAR} $@ --hist_files ${HIST_FILES} --fut_files ${FUTURE_FILES} --hist_time_bounds ${BASE_START}-01-01 ${BASE_END}-12-31 --fut_time_bounds ${FUTURE_START}-01-01 ${FUTURE_END}-12-31 --method ${METHOD} --input_units ${MODEL_UNITS} --output_units ${OUTPUT_UNITS} 
