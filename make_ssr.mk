...
ssr-hist : ${HIST_DATA}
${HIST_DATA} : 
	${PYTHON} ${CODE_DIR}/ssr.py ${HIST_FILES_ORIG} ${HIST_VAR} $@ --time_bounds ${HIST_START}-01-01 ${HIST_END}-12-31 --input_units ${HIST_UNITS} --output_units ${OUTPUT_UNITS}

## ssr-ref: Apply Singularity Stochastic Removal to reference data
ssr-ref : ${REF_DATA}
${REF_DATA} : 
	${PYTHON} ${CODE_DIR}/ssr.py ${REF_FILES_ORIG} ${REF_VAR} $@ --time_bounds ${REF_START}-01-01 ${REF_END}-12-31 --input_units ${REF_UNITS} --output_units ${OUTPUT_UNITS}

## ssr-target: Apply Singularity Stochastic Removal to target data
ssr-target : ${TARGET_DATA}
${TARGET_DATA} : 
	${PYTHON} ${CODE_DIR}/ssr.py ${TARGET_FILES_ORIG} ${TARGET_VAR} $@ --time_bounds ${TARGET_START}-01-01 ${TARGET_END}-12-31 --input_units ${TARGET_UNITS} --output_units ${OUTPUT_UNITS}
